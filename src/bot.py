...
async def add_reminder(pool, user_id, chat_id, text, remind_time):
    async with pool.acquire() as conn:
        await conn.execute(
            ...
            user_id, chat_id, text, remind_time
        )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    parts = text.split(' ', 2)
    if len(parts) < 3:
        await update.message.reply_text("Не понял формат. Попробуй: `25.12 15:30 Купить молоко`")
    ...
    try:
        ...
        await update.message.reply_text("Ошибка в дате/времени. Используй формат ДД.ММ.ГГГГ ЧЧ:ММ или 'завтра ЧЧ:ММ'")
...
async def done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if num < 1 or num > len(reminders):
        ...
        logger.warning(f"Пользователь {user_id} указал несуществующий номер задачи: {num}")
        return
